...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[NewExpressmaint]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[NewExpressmaint]
GO

CREATE PROCEDURE [dbo].[NewExpressmaint]
(
...
/* 
   ...
   17/07/2014     Neil Ennis              modified file name to comply with old Expressmaint version
...
************************/
   ...
   DECLARE @jobstart        char(13)         ; 
   DECLARE @stage           int              ; SET @stage        = 1
   DECLARE @compressok      bit              ; SET @compressok   = 0
   DECLARE @versionmajor    int              
   DECLARE @engineedition   int

   SET @jobstart = CONVERT(char(8),@jobdt,112)+'_'+LEFT(REPLACE(CONVERT(char(8),@jobdt,108),':',''),4)   
...
************************/
   ...
   IF @versionmajor<9
	BEGIN                   				
   	RAISERROR('SQL2005 or higher is required for sp_NewExpressmaint',16,1)
      SET @ret = 1
   	GOTO CLEANUP	
	END
...
************************************/
   ...
   BEGIN
      ...
      SET @output = 'NewExpressmaint utility, Logged on to SQL Server [' + @@SERVERNAME + '] as ' + '[' + @user + ']'
...
************************/
   ...
   ************************/
   ...
   BEGIN
	  ...
	  -- adjust for new file name (hyphen before time)
	  UPDATE #files SET filename = SUBSTRING(fileName,1,LEN(fileName)-CHARINDEX('_',REVERSE(filename)))+SUBSTRING(filename,LEN(filename)-CHARINDEX('_',REVERSE(filename))+2,99)
      ...
      IF @debug = 1 SELECT 'remaining',* FROM #files
      ...
      BEGIN
         ...
         IF @delcount>0
         BEGIN
		    UPDATE #files SET filename = SUBSTRING(fileName,1,LEN(fileName)-CHARINDEX('.',REVERSE(filename))-4)+'_'+SUBSTRING(filename,LEN(filename)-CHARINDEX('.',REVERSE(filename))-3,99)
   ...
   ************************/
   ...
   -- remove hyphen before time to match new logic
   UPDATE #files SET filename = SUBSTRING(fileName,1,LEN(fileName)-CHARINDEX('_',REVERSE(filename)))+SUBSTRING(filename,LEN(filename)-CHARINDEX('_',REVERSE(filename))+2,99)
   ...
   BEGIN
      ...
      IF @delcount>0
      BEGIN
	   -- replacce hyphen before time to delete properly
		    UPDATE #files SET filename = SUBSTRING(fileName,1,LEN(fileName)-CHARINDEX('.',REVERSE(filename))-4)+'_'+SUBSTRING(filename,LEN(filename)-CHARINDEX('.',REVERSE(filename))-3,99)
...
************************/
   ...
   IF @delbkflag<>0
   BEGIN
      SET @errormsg = 'NewExpressmaint encountered errors deleting old backup files' + CHAR(13)
   ...
   BEGIN
      SET @errormsg = 'NewExpressmaint encountered errors deleting old report files' + CHAR(13)
                    + CASE WHEN @report = 1 THEN ('Please see ' + @reportfilename + CHAR(13) + ' for further details') ELSE '' END
      RAISERROR(@errormsg,16,1)
      SET @ret = 1
   END
   ...
   BEGIN
      ...
      EXEC sp_OAMethod @file,'WriteLine',NULL,''
      SET @output = 'NewExpressmaint processing finished at ' + CONVERT(varchar(25),GETDATE(),100) 
